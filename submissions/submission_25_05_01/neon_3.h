...
{
  /**
   * @brief Matmul that loops over the K dimension of an original matmul of (M=16, N=6, K=1) now with loop over K=64.
   ...
   **/
  void matmul_16_6_64(float const *a, float const *b, float *c, int64_t lda, int64_t ldb, int64_t ldc);

  /**
   * @brief Matmul that loops over the K and M dimension of an original matmul of (M=16, N=6, K=1) now with loop over
   * K=64 and M=64=4*16.
   * @param a pointer to column-major matrix A.
   * @param b pointer to column-major matrix B.
   * @param c pointer to column-major matrix C.
   ...
   * @param ldc leading dimension of C.
   **/
  void matmul_64_6_64(float const *a, float const *b, float *c, int64_t lda, int64_t ldb, int64_t ldc);

  /**
   * @brief Matmul that loops over the K, M and N dimension of an original matmul of (M=16, N=6, K=1) now with loop over
   * K=64, M=64=4*16 and N=48=8*6.
   * @param a pointer to column-major matrix A.
   * @param b pointer to column-major matrix B.
   * @param c pointer to column-major matrix C.
   * @param lda leading dimension of A.
