...
int main()
{
  ...
  std::cout << "Executed " << run_instructions_latency_fmla_4s_source << " \"FMLA 4s\" Instructions in "
            << diff_latency_fmla_4s_source.count() << " milliseconds on a single Unit." << std::endl
            << "Resulting in a Throughput of " << run_instructions_latency_fmla_4s_source / diff_latency_fmla_4s_source.count() * 1000
  ...
  const std::chrono::duration<double, std::milli> diff_latency_fmla_4s_destination =
    end_latency_fmla_4s_destination - start_latency_fmla_4s_destination;
  std::cout << "Executed " << run_instructions_latency_fmla_4s_destination << " \"FMLA 4s\" Instructions in "
            << diff_latency_fmla_4s_destination.count() << " milliseconds on a single Unit." << std::endl
            << "Resulting in a Throughput of "
            << run_instructions_latency_fmla_4s_destination / diff_latency_fmla_4s_destination.count() * 1000 << " Instructions per Second!"
            << std::endl
            << std::endl;
