...
BENCHMARK_TEMPLATE_DEFINE_F(GemmMxNxKFixture, BM_matmul_16_6_1, 16, 6, 1)(benchmark::State &state)
{
  // Generate kernel
  mini_jit::Kernel native_kernel;
  mini_jit::kernels::matmul_16_6_1(native_kernel);
  ...
  flops = (16 * 6 * 1) * 2;  // M * N * K * 2 instructions (add & mul)
  flops *= state.iterations();
};

BENCHMARK_REGISTER_F(GemmMxNxKFixture, BM_matmul_16_6_1)->MinWarmUpTime(1.0);  // WarmUp in seconds