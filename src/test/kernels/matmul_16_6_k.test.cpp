...
TEST_CASE("Test matmul_16_6_k (M=16, N=6, K=1) jited gemm correctness counting data", "[jit][neon_3][correctness][gemm]")
{
  float matrix_a[16 * 1];
  float matrix_b[1 * 6];
  float matrix_c[16 * 6];
...
{
  float matrix_a[16 * 128];
  float matrix_b[128 * 6];
  float matrix_c[16 * 6];
  float matrix_c_verify[16 * 6];
  ...
  // Generate kernel
  mini_jit::Brgemm brgemm;
  brgemm.generate(16, 6, 128, 1, 0, 0, 0, mini_jit::Brgemm::dtype_t::fp32);
  mini_jit::Brgemm::kernel_t kernel = brgemm.get_kernel();

  // Run matmuls
  kernel(matrix_a, matrix_b, matrix_c, 16, 128, 16, 1, 1);
  naive_matmul_M_N_K<16, 6, 128>(matrix_a, matrix_b, matrix_c_verify, 16, 128, 16);

  verify_matmul(matrix_c_verify, matrix_c);
}

TEST_CASE("Test matmul_16_6_k (M=16, N=6, K=128) jited gemm correctness counting data", "[jit][neon_3][correctness][gemm]")
{
  float matrix_a[16 * 128];
  float matrix_b[128 * 6];
  float matrix_c[16 * 6];
