...
BENCHMARK_DEFINE_F(UnaryFixture, BM_unary_zero)(benchmark::State &state)
{
  int M = state.range(0);
  int N = state.range(1);

  mini_jit::Kernel native_kernel;
  mini_jit::kernels::unary_zero(native_kernel, M / 16, N, M % 16);
...
BENCHMARK_REGISTER_F(UnaryFixture, BM_unary_zero)
  ->ArgNames({"M", "N"})
