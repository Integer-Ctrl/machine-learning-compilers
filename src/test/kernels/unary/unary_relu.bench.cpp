...
BENCHMARK_DEFINE_F(UnaryFixture, BM_unary_relu)(benchmark::State &state)
{
  int M = state.range(0);
  int N = state.range(1);

  mini_jit::Kernel native_kernel;
  mini_jit::kernels::unary_relu(native_kernel, M, N);
...
BENCHMARK_REGISTER_F(UnaryFixture, BM_unary_relu)
  ->ArgNames({"M", "N"})
