...
void GenerationTest::naive_matmul_M_N_K_Batch(const float* __restrict__ a, const float* __restrict__ b,
    float* __restrict__ c, int64_t lda, int64_t ldb, int64_t ldc, int64_t batch_stride_a, int64_t batch_stride_b)
{
...
GenerationTest::GenerationTest(uint32_t M, uint32_t N, uint32_t K)
    : GenerationTest(M, N, K, 1)
{
}
    ...
    const uint32_t batch_stride_b)
{
    ...
    naive_matmul_M_N_K_Batch(matrix_a, matrix_b, matrix_c_verify, lda, ldb, ldc,
