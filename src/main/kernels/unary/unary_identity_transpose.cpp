...
void transpose_in_1x1(mini_jit::Kernel &kernel, const uint32_t m_loop, const uint32_t n_loop, mini_jit::kernels::internal::ops_t ops,
                      char const *path)
{
  ...
  kernel.add({
    ...
    // iterator for the n loop
    mov(x16, n_loop),
    // n-loop
    sub(x16, x16, 1),

    mov(x4, x0),  // Working register for the x0
    mov(x5, x1),  // Working register for the x1

    // iterator for the m loop
    mov(x17, m_loop),
    // m-loop
    sub(x17, x17, 1),

    ldrPost(s0, x4, 4),  // += sizeof(float)
  });
  uint32_t n_jump_start = kernel.get_instruction_count() - 6;
  uint32_t m_jump_start = kernel.get_instruction_count() - 2;

  ops(kernel, v0);

  uint32_t m_jump_end = kernel.get_instruction_count() + 1;
  uint32_t n_jump_end = kernel.get_instruction_count() + 4;
  kernel.add({
    st1Post(s0, 0, x5, x3),  // += ldb
                                                      ...
                                                      char const *path)
{
  ...
  else
  {
    ...
    transpose_in_1x1(kernel, m_loop, n_loop, ops, path);
  ...
  if ((static_cast<int64_t>(n_loop / n_transpose_block) - 1) > 0 && n_loop > (n_transpose_block * 2))
  {
    ...
    transpose_axis(kernel, m_transpose_block, n_transpose_block, ops);
    ...
    if (n2_m_loops > 0)
    {
      ...
      transpose_else(kernel, m_transpose_block, n_transpose_block, ops);
    ...
    {
      transpose_else(kernel, m_transpose_rest, n_transpose_block, ops);
    }
  ...
  if (n_loop / n_transpose_block > 0 && n_loop > n_transpose_block)
  {
    ...
    kernel.add({
      ...
      madd(x10, x8, x12, x10),  // matrix_a: x10 += m_transpose_block * sizeof(float)
      madd(x11, x3, x12, x11),  // matrix_b: x11 += ldb * m_transpose_block
      madd(x11, x8, x13, x11),  // matrix_b: x11 += m_transpose_block * sizeof(float)
  ...
  transpose_axis(kernel, m_transpose_rest, n_transpose_rest, ops);
...
void mini_jit::kernels::internal::transpose_axis(mini_jit::Kernel &kernel, const uint32_t m, const uint32_t n, ops_t ops)
{
  using namespace mini_jit::arm_instructions;
...
void mini_jit::kernels::internal::transpose_else(mini_jit::Kernel &kernel, const uint32_t m, const uint32_t n, ops_t ops)
{
  using namespace mini_jit::arm_instructions;
