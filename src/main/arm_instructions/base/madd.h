...
namespace mini_jit
{
  namespace arm_instructions
  {
    namespace internal
    {
      ...
      constexpr uint32_t madd(const uint32_t Rd, const uint32_t Rn, const uint32_t Rm, const uint32_t Ra, const bool is64bit)
      {
    ...
    {
      return internal::madd(static_cast<uint32_t>(Wd), static_cast<uint32_t>(Wn), static_cast<uint32_t>(Wm), static_cast<uint32_t>(Wa),
                            false);
    }

    constexpr uint32_t madd(const R64Bit Xd, const R64Bit Xn, const R64Bit Xm, const R64Bit Xa)
    {
      return internal::madd(static_cast<uint32_t>(Xd), static_cast<uint32_t>(Xn), static_cast<uint32_t>(Xm), static_cast<uint32_t>(Xa),
                            true);
    }
